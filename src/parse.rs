...
pub enum DownloadMode {
    Chapter { url: String, format: Format },
...
impl std::convert::TryFrom<&CliArguments> for DownloadMode{
    ...
    fn try_from(value: &CliArguments) -> Result<Self, Self::Error> {
        let url =&value.url;
        let format = value.format.clone();
        #[cfg(feature = "shuba")]
        if let Some(mode) = is_shuba(url,format) {
            return Ok(mode);
        }
...
#[cfg(feature = "shuba")]
fn is_shuba(value: &str, format: Format) -> Option<DownloadMode> {
